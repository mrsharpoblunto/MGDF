...
/**
 The whole point of having a separate DLL for some trivial test helper classes
 is that it ensures that the load path for dependent DLL's is set correctly when
 loading a game module. This is important for modules that actually have
 dependencies on 3rd party dlls
 */
...
// this define is set if we're currently building a core component dll that is
// used only within the core select import or export depending on it

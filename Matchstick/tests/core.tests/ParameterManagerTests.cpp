...
SUITE( ParameterManagerTests )
{
	TEST( TestParseParameters ) {
		ParameterManager::Instance().AddParameterString("-parameter value -novalueparam -parameter1 value1 -parameter2 \"quoted param\" -parameter3 'quoted param'");
		...
		CHECK_EQUAL( "quoted param", ParameterManager::Instance().GetParameter( "parameter2" ) );
		CHECK_EQUAL( "quoted param", ParameterManager::Instance().GetParameter( "parameter3" ) );
	}
}

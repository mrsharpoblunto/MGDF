...
SUITE(VFSTests) {
  struct VFSTestFixture {
    VFSTestFixture() {
      ...
      _vfs->RegisterArchiveHandler(
          zip::CreateZipArchiveHandlerImpl((IErrorHandler *)_errorHandler));
    }
  ...
  */
  TEST_FIXTURE(VFSTestFixture, ZipArchiveTests) {
    ...
    CHECK_WS_EQUAL(L"preferences.xml",
                   _vfs->GetFile(L"preferences.xml")->GetName());
    CHECK_WS_EQUAL(L"preferenceTemplates.xml",
                   _vfs->GetFile(L"preferenceTemplates.xml")->GetName());
    CHECK_WS_EQUAL(L"gameState.xml",
                   _vfs->GetFile(L"boot/gameState.xml")->GetName());
    CHECK_WS_EQUAL(L"persistency.xml",
                   _vfs->GetFile(L"boot/persistency.xml")->GetName());
  ...
  */
  ...
  class ContainsFilter : public MGDF::IFileFilter {
   public:
    ContainsFilter(std::wstring match) : _match(match) {}
  ...
  */
  TEST_FIXTURE(VFSTestFixture, FileSystemTests) {
    ...
    CHECK_WS_EQUAL(L"preferences.json",
                   _vfs->GetFile(L"preferences.json")->GetName());
    CHECK_WS_EQUAL(L"gameState.json",
                   _vfs->GetFile(L"gameState.json")->GetName());

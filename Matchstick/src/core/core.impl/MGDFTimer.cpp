...
const char *CounterBase::GetName() const
{
	return _name.c_str();
}

ULONG CounterBase::AddRef()
{
	return ++_references;
}
...
CPUPerformanceCounter::CPUPerformanceCounter( const char *name, UINT32 maxSamples, Timer& timer, LARGE_INTEGER frequency )
...
{
	if ( _started ) {
		LOG( "Call to CPU Timer Begin for Timer " << GetName() << " that has already started timing", LOG_ERROR );
	} else {
		_started = true;
...
{
	if ( !_started ) {
		LOG( "Call to GPU Timer End without matching call to Begin on Timer " << GetName(), LOG_ERROR );
	} else {
		_started = false;
...
GPUPerformanceCounter::GPUPerformanceCounter( const char *name, UINT32 maxSamples, Timer& timer )
	: CounterBase( name, maxSamples, timer )
...
void GPUPerformanceCounter::Begin()
{
	...
	if ( _hasRun ) {
		LOG( "GPU Timer " << GetName() << " has already been used this frame", LOG_HIGH );
	} 
	else if ( _started ) {
		LOG( "Call to GPU Timer Begin for Timer " << GetName() << " that has already started timing", LOG_ERROR );
	}
		...
		if ( !_beginQueries.empty() ) {
			...
			_pendingQueries.insert( std::pair<ID3D11Query *,std::pair<ID3D11Query *,ID3D11Query *>>( 
				_currentDisjoint, 
				std::pair<ID3D11Query *,ID3D11Query *>( _beginQueries.top(), _endQueries.top() ) ) );
...
{
	if ( _started ) {
		LOG( "GPU Timer " << GetName() << " that was Begun this frame was not Ended this frame - forcing End", LOG_ERROR );
		End();
	}
}

void GPUPerformanceCounter::Init( ID3D11Device *device, ID3D11DeviceContext *context, UINT32 bufferSize )
{
	_ASSERTE( device );
	_ASSERTE( context );
...
void GPUPerformanceCounter::DataReady( ID3D11Query *disjoint, UINT64 frequency )
{
	...
	if ( _context->GetData( it->second.first, &timeStampBegin, sizeof( UINT64 ), 0 ) != S_OK ) {
		LOG( "Failed to bet Begin Data for GPU Timer " << GetName() << "- Ignoring sample", LOG_ERROR );
		success = false;
	}

	UINT64 timeStampEnd;
	if ( success && _context->GetData( it->second.second, &timeStampEnd, sizeof( UINT64 ), 0 ) != S_OK ) {
		LOG( "Failed to bet End Data for GPU Timer " << GetName() << "- Ignoring sample", LOG_ERROR );
		success = false;
	}
...
MGDFError Timer::CreateCPUCounter( const char *name, IPerformanceCounter **counter )
{
	...
	CPUPerformanceCounter *c = new CPUPerformanceCounter( name, _maxSamples, *this, _freq );
...
MGDFError Timer::CreateGPUCounter( const char *name, IPerformanceCounter **counter )
{
	...
	if (!name ) return MGDF_ERR_INVALID_TIMER_NAME;
	GPUPerformanceCounter *c = new GPUPerformanceCounter( name, _maxSamples, *this );
...
void Timer::Begin()
{
	if ( _gpuTimersSupported ) {
		if ( !_pendingQueries.empty() ) {
			...
			if ( _context->GetData( query, &disjoint, sizeof( D3D11_QUERY_DATA_TIMESTAMP_DISJOINT ), 0 ) == S_OK ) {
				std::lock_guard<std::mutex> lock( _mutex );

...
ZipArchive::ZipArchive(IErrorHandler *errorHandler)
    : _errorHandler(errorHandler), _root(nullptr) {
  _ASSERTE(errorHandler);
}

ZipArchive::~ZipArchive() {
  if (_zip) unzClose(_zip);
}

ZipFileRoot *ZipArchive::MapArchive(const wchar_t *name,
  ...
  if (_zip) {
    ...
    for (INT32 ret = unzGoToFirstFile(_zip); ret == UNZ_OK;
         ret = unzGoToNextFile(_zip)) {
      unz_file_info info;
      char name[FILENAME_BUFFER];

      unzGetCurrentFileInfo(_zip, &info, name, FILENAME_BUFFER, nullptr, 0,
                            nullptr, 0);
      ...
      // this means that the entire folder tree will be created in the case of
      // folders, and that the last element will be excluded for files which is
      ...
      if (info.uncompressed_size > 0) {
        ...
        ZipFileImpl *zipFile =
            new ZipFileImpl(parentFile, this, std::move(header));
...
IFile *ZipArchive::CreateParentFile(std::wstring &path, IFile *root,
...
MGDFError ZipArchive::GetFileData(ZipFileHeader &header, ZipFileData &data) {
  ...
  if (header.size > UINT32_MAX) {
    ...
    LOG("Archive files cannot be over 4GB in size "
            << Resources::ToString(header.name),
        LOG_ERROR);
  ...
  if (unzReadCurrentFile(_zip, data.data, static_cast<UINT32>(header.size)) <
      0) {
    result = MGDF_ERR_INVALID_ARCHIVE_FILE;
    goto cleanup;
  }

...
public:
	...
	virtual INT64 GetSize() { return _header.size; }

	virtual time_t GetLastWriteTime() const
	{
		return _handler->GetArchiveRoot()->GetLastWriteTime();
	}
	virtual const wchar_t *GetArchiveName() const { return _handler->GetArchiveRoot()->GetName(); }
	virtual const wchar_t *GetPhysicalPath() const { return _handler->GetArchiveRoot()->GetPhysicalPath(); }
	virtual const wchar_t *GetName() const { return _header.name.c_str(); }
private:
	ZipArchive *_handler;

...
public:
	...
	*/
	...
	/**
	create a sound from a file in the VFS returns NULL if the sound could not be created (see system->GetLastError() to get details as to why it failed)
	...
	*/
	...
	will deactivate a sound source if possible, however if no sources are available then this function returns NULL 
	also returns NULL if the stream could not be created (see system->GetLastError() to get details as to why it failed)
	\param file the data source for the sound
	*/
